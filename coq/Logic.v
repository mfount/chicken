...
Theorem iff_trans : forall P Q R : Prop, 
  (P <-> Q) -> (Q <-> R) -> (P <-> R).
Proof.
  intros. split.  destruct H. destruct H0. intros. apply H0. apply H. apply H3.
  intros. destruct H. destruct H0. apply H2. apply H3. apply H1.
Qed.
...
Proof.
  ...
  left. apply H. left. apply H. destruct H0. left. apply H0. right. split. apply H.
  apply H0.
Qed.
(** [] *)
...
Theorem or_distributes_over_and : forall P Q R : Prop,
  P \/ (Q /\ R) <-> (P \/ Q) /\ (P \/ R).
Proof.
  intros. split. intros. destruct H. split. left. apply H. left. apply H.
  split. destruct H. right. apply H. destruct H. right. apply H0.
...
Proof. 
  intros. destruct b.
  Case "b = true". destruct c. simpl in H. inversion H. right. reflexivity.
...
Proof.
  ...
  Case "b = false". destruct c. right. reflexivity.
  inversion H.
Qed.
...
Proof.
  intros.
  unfold not. intros. apply H. right. intros. destruct H. left. apply H0.
Qed.
...
Theorem false_beq_nat : forall n m : nat,
     ...
     beq_nat n m = false.
Proof. 
  intros. unfold not in H. generalize dependent m. induction n as [ | n' ].
  ...
  Case "n = S n'".
    ...
    SCase "m = S m'". simpl. apply IHn'. intros. apply H. apply f_equal. apply H0.
Qed.
